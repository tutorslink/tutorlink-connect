...
function AdminPages() {
  ...
  const [form, setForm] = useState({ id: "", title: "", slug: "", content: "", seo_title: "", seo_description: "", status: "draft" });
  ...
  const openEditor = (page?: any) => {
    if (page) {
      setEditing(page);
      setForm({ id: page.id, title: page.title, slug: page.slug, content: page.content || "", seo_title: page.seo_title || "", seo_description: page.seo_description || "", status: page.status || "draft" });
    } else {
      setEditing(null);
      setForm({ id: "", title: "", slug: "", content: "", seo_title: "", seo_description: "", status: "draft" });
    }
    setShowEditor(true);
  };
  ...
  return (
    <div>
      <PageHeader title="Static Pages" description="Manage website content pages." action={
        <Button className="gap-2" onClick={() => openEditor()}><Plus className="h-4 w-4" /> New Page</Button>
      } />
          ...
          {pages.map((p) => (
            <Card key={p.id}>
              <CardContent className="p-5 space-y-3">
                ...
                <Button variant="outline" size="sm" className="w-full gap-1.5" onClick={() => openEditor(p)}>
      ...
      <Dialog open={showEditor} onOpenChange={setShowEditor}>
        <DialogContent className="sm:max-w-[600px]">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Content</Label>
              <Textarea value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} rows={8} placeholder="Page content..." />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label>SEO Title</Label>
                <Input value={form.seo_title} onChange={(e) => setForm({ ...form, seo_title: e.target.value })} placeholder="SEO title" />
              </div>
              <div className="space-y-2">
                <Label>Status</Label>
            ...
            <div className="space-y-2">
              ...
              <Textarea value={form.seo_description} onChange={(e) => setForm({ ...form, seo_description: e.target.value })} rows={2} placeholder="Meta description for search engines" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowEditor(false)}>Cancel</Button>
            <Button onClick={handleSave} className="gap-2"><Save className="h-4 w-4" /> Save Page</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
