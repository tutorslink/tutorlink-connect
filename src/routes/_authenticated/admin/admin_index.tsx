...
import { Users, GraduationCap, FileText, Star, Bell, DollarSign, TrendingUp, BookOpen, ShieldAlert } from "lucide-react";
      ...
      {loading || !metrics ? (
        ...
        <>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <StatCard icon={FileText} label="Pending Applications" value={metrics.pendingTutorApplications} color="text-amber-600 bg-amber-50 dark:bg-amber-950/30" />
            <StatCard icon={Star} label="Avg Rating" value={metrics.averageTutorRating || "—"} color="text-purple-600 bg-purple-50 dark:bg-purple-950/30" />
            <StatCard icon={Bell} label="Total Reviews" value={metrics.totalReviews} color="text-rose-600 bg-rose-50 dark:bg-rose-950/30" />
            ...
            />
            <StatCard icon={BookOpen} label="Completed Lessons" value={metrics.completedLessons} color="text-indigo-600 bg-indigo-50 dark:bg-indigo-950/30" />
          </div>
              ...
              <CardContent>
                {recentApps.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {recentApps.map((app) => (
                      <div key={app.id || app.$id} className="flex items-center justify-between p-3 border rounded-lg">
                        <div>
                          <p className="font-semibold text-sm">{app.full_name || app.fullName || "Applicant"}</p>
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`text-sm font-medium ${metrics.aiAssistantEnabled ? "text-emerald-600" : "text-red-600"}`}>
                  ...
                  <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Pending Recruitment</span><span className="text-sm font-medium">{metrics.pendingRecruitmentApplications}</span></div>
                </div>
              </CardContent>
            </Card>
          </div>
